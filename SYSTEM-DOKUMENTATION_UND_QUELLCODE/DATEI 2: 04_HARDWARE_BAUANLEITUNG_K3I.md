...
* **Das Gehirn:** Einen kleinen, billigen Mikrocontroller (z. B. Arduino Nano oder ESP32). Er fungiert als "Kern 3" und liest das Signal vom PC aus.
* **Die elektronische Bremse (Stufe 1 - 1 Millisekunde):** Einen N-Kanal MOSFET-Transistor (Typen-Empfehlung: IRLZ44N – der schaltet extrem schnell und lässt sich direkt mit 5V ansteuern).
* **Die physische Trennung (Stufe 2 - 12 Millisekunden):** Ein schnelles 5V-Sicherheitsrelais (oder ein kleines Signalrelais mit Wechselkontakt).
* **Zubehör:** Eine Standard-Lochrasterplatine, ein paar Widerstände (10k Ohm als Pull-Down), zwei RJ45-Netzwerkbuchsen (zum Durchschleifen des Kabels) und eine externe Stromquelle (z. B. 5V-Netzteil).
...
* **Das Signal vom PC:** Das Signal (Keep-Alive) kommt aus dem geimpften Kern über eine Datenleitung am Arduino an.
* **Der MOSFET (Stufe 1):** Verbinde den Steuerpin (Gate) des MOSFETs mit dem Ausgang des Arduinos. Die zu schützende Datenleitung wird an den Schalter (Drain/Source) des MOSFETs gehängt.
...
* **Der Relais (Stufe 2):** Hänge die Spule des mechanischen Relais parallel an den gleichen Steuerstromkreis. Die Datenleitung wird zusätzlich durch den mechanischen Schaltkontakt (Wechsler) des Relais geschleift.
...
## 3. Der Steuerungs-Code für die Hardware (Arduino)
